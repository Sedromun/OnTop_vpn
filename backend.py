...
async def get_vless_perm_keys(order_id_enc: str):
    ...
    return HTMLResponse(key, headers={"Profile-Title": "Clique VPN",
                                      "Subscription-Userinfo": f'upload=17477; download=113814; total=0; expire={str(expiry_time / 1000)}'})


# app.mount("/web3", StaticFiles(directory="static", html=True), name="static")
# app.include_router(connection_router, prefix="/web3")


# @app.get("/web3/{path:path}")
# async def html_landing() -> HTMLResponse:
#     return HTMLResponse(prebuilt_html(title="Clique-VPN", api_root_url="/"))
